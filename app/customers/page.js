...
export default function CustomersPage() {
  ...
  async function handleSave() {
    ...
    if (mode === "add") {
      const { error: err } = await getSupabase().from("customers").insert({
        name: form.name.trim(),
      ...
      const { error: err } = await getSupabase().from("customers").update({
        name: form.name.trim(),
        wechat_id: form.wechat_id.trim() || null,
        phone_number: form.phone_number.trim() || null,
        shipping_address: form.shipping_address.trim() || null,
  ...
  async function handleDelete(id, name) {
    if (!confirm(`Delete customer "${name}"? This will also delete all their orders.`)) return;
  ...
  return (
    <main className="min-h-screen bg-white p-6 max-w-3xl mx-auto">
      ...
      {mode && (
        <div className="border-2 border-gray-800 rounded-xl p-6 mb-8">
          ...
          <div className="grid grid-cols-1 gap-4">
            <Field label="Name *">
              <input type="text" value={form.name} onChange={(e) => set("name", e.target.value)} className={inputCls} placeholder="Full name" />
            </Field>
            <Field label="WeChat ID">
              <input type="text" value={form.wechat_id} onChange={(e) => set("wechat_id", e.target.value)} className={inputCls} placeholder="WeChat ID" />
            </Field>
            <Field label="Phone Number">
              <input type="text" value={form.phone_number} onChange={(e) => set("phone_number", e.target.value)} className={inputCls} placeholder="+49 123 456789" />
            </Field>
            <Field label="Shipping Address">
              <textarea rows={3} value={form.shipping_address} onChange={(e) => set("shipping_address", e.target.value)} className={inputCls} placeholder="Full shipping address" />
            </Field>
          </div>
      ...
      {/* Customer List */}
      {loading ? (
        <p className="text-gray-400 text-base">Loading customers...</p>
      ) : customers.length === 0 ? (
        <p className="text-gray-400 text-base">No customers yet. Add one above.</p>
      ) : (
        <div className="flex flex-col gap-3">
          {customers.map((c) => (
            <div
              key={c.id}
              className={`border-2 rounded-xl p-5 ${editingId === c.id ? "border-black bg-gray-50" : "border-gray-800 bg-white"}`}
            >
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <div className="mt-2 flex flex-col gap-1 text-base text-gray-600">
                    {c.wechat_id && <p>WeChat: {c.wechat_id}</p>}
                    {c.phone_number && <p>Phone: {c.phone_number}</p>}
                    {c.shipping_address && <p>Address: {c.shipping_address}</p>}
                  </div>
                </div>
